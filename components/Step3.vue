            ...
            <div class="ma-auto position-relative" style="max-width: 500px">
                <v-container class="info-container2">
                    <div class="p ma-auto position-relative">
                        <p>Ya casi terminas, por favor diligencia <br>la siguiente información para generar <br>tu
                            diagnóstico.</p>
                    </div>
                </v-container>
            </div>
            <v-col class="d-flex" cols="13" sm="13">
                <v-text-field v-model="formattedIncome" label="Ingresos mensuales" @input="formatIncome"
                    prepend-inner-icon="mdi-cash" required outlined />
            </v-col>

            <v-col class="d-flex" cols="13" sm="13">
                <v-text-field v-model="formattedExpenses" label="Gastos mensuales" @input="formatExpenses"
                    prepend-inner-icon="mdi-cash" required outlined />
            </v-col>
            <!-- Ahorros -->
            ...
            <v-row>
                <v-col class="d-flex" cols="12" sm="12">
                    <v-select v-model="ahorros" :items="ahorrosOpciones.map(option => option.text)" label="¿Tienes ahorros?"
                        hide-details outlined required></v-select>
                </v-col>
                <v-col class="d-flex" cols="12" sm="12" v-if="ahorros === 'SI'">
                    <v-text-field v-model="formattedSavings" label="Valor de tus ahorros" @input="formatSavings"
                        @blur="formatSavings" prepend-inner-icon="mdi-piggy-bank-outline" hide-details outlined required />
                </v-col>
            </v-row>

            <v-container class="info-container">
                <div class="p position-relative">
                    <p>Para terminar, por favor, indícanos <br>tu <b>objetivo.</b> <br>Una vez no tengas reportes
                        negativos <br>en centrales de riesgo. <br>¿Cuál seria tu <b>meta financiera</b>?</p>
                </div>
            </v-container>

            <!-- Obetivo financiero -->
            <v-select v-model="selectedOption" :items="options" label="Objetivo Financiero"
                prepend-inner-icon="mdi-coffee-outline" outlined></v-select>
        </v-container>
    </v-form>
</template>
...
<script>
import Step1 from '~/components/Step3.vue'
